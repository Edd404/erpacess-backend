    ...
    try {
      ...
      if (orderData.warranty_months > 0 && warrantyExpiry) {
        ...
        doc.save().fillColor(C.green).fontSize(13).font('Helvetica-Bold')
          .text(`${orderData.warranty_months} ${orderData.warranty_months === 1 ? 'Mês' : 'Meses'} de Garantia`, ML + 44, y + 10).restore();
      ...
      if (userNotes) {
        ...
        rect(doc, ML, y, CW, 'auto', C.amberBg, 6);
        const notesStartY = y;
        doc.save().fillColor(C.gray1).fontSize(9).font('Helvetica')
          .text(userNotes, ML + 10, y + 8, { width: CW - 20 });
        const notesEndY = doc.y + 8;
        rect(doc, ML, notesStartY, CW, notesEndY - notesStartY, C.amberBg, 6);
        line(doc, ML, notesStartY, ML, notesEndY, C.amber, 3);
        // Redraw text on top
        doc.save().fillColor(C.gray1).fontSize(9).font('Helvetica')
          .text(userNotes, ML + 14, notesStartY + 8, { width: CW - 24 }).restore();
        y = notesEndY + 12;
      }
